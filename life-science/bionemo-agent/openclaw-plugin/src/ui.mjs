...
export function createUiHandlers({ store, env = process.env, runtimeVersion }) {
  return {
    ...
    async readiness(_req, res) {
      ...
      const configured = { reasoning: state.reasoning, reasoningProvider: state.reasoningProvider, modelBackend: state.modelBackend, nvidia: state.nvidia, nebius: state.nebius, mcp: state.mcp, tavily: state.tavily, gateway: Boolean(env.OPENCLAW_GATEWAY_TOKEN) };
