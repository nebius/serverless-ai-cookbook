...
export const WORKBENCH_EXAMPLE_SESSIONS = Object.freeze([
  example({
    ...
    steps: [
      ...
      "Explain sanitized inventory, credential, and research-use boundaries",
    ],
    prompt: "Give me a concise orientation to this OpenClaw BioNeMo workbench using only the source-owned instructions already in context. Do not call any tool. Explain that this starter is static until I send it; distinguish the reasoning model from the BioNeMo scientific backend; summarize native skills, the local ClawBio catalog, up to ten clean atomic bionemo_* tools, seven clean composed workflows, notebooks, and generated artifacts. Explain that bionemo_molmim, bionemo_openfold2, bionemo_openfold3, and four composed demos select the configured backend; NVIDIA-only wrappers are hidden without that credential. Explain that bionemo_models_list is the browser's sanitized read-only inventory tool, while raw hosted model, job, status, fetch, and capability operations remain private and absent. Keep the explanation research-only. Do not infer credentials, provider health, current model availability, or runtime configuration that is not visible, and do not ask for a secret.",
  ...
  example({
    ...
    steps: [
      ...
      "Summarize ready and non-ready counts without inferring capabilities",
      "Confirm that no model job or scientific compute was submitted",
    ],
    prompt: "Inspect the configured BioNeMo model inventory in read-only mode. Call bionemo_models_list exactly once with no arguments. Do not call any other bionemo_* tool, any ClawBio or Tavily tool, or any raw hosted model, job, status, fetch, or capability operation, and do not retry. Report the returned model count and ready count, then list each returned model's sanitized id, displayName, family, and readiness. Explicitly state that this inventory call submitted no scientific compute or model job. Do not infer that an inventory entry has a browser compute wrapper or that readiness establishes scientific validity. If the inventory tool reports an authentication, entitlement, quota, rate-limit, or availability error, report only that safe category and stop.",
