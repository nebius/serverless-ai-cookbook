...
export const WORKBENCH_EXAMPLE_SESSIONS = Object.freeze([
  example({
    ...
    prompt: "Give me a concise orientation to this OpenClaw BioNeMo workbench using only the source-owned instructions already in context. Do not call any tool. Explain that this starter is static until I send it; distinguish the reasoning model from the BioNeMo scientific backend; summarize native skills, the local ClawBio catalog, bounded bionemo_* convenience wrappers, seven composed workflows, notebooks, and generated artifacts. If bionemo_models__* tools are displayed, also explain that they are the complete adapted MCP compute surface for all 16 inventory services and that product-neutral MCP helpers provide model description, resumable upload, cross-job listing, status, and artifact fetch without exposing credentials, remote-host local-path staging, or upstream clawbio_* compatibility names. Explain that bionemo_models_list is the sanitized read-only inventory wrapper when displayed. Keep the explanation research-only. Do not infer provider health or current model availability without a tool result, and do not ask for a secret.",
  ...
  example({
    ...
    description: "A no-tool explanation of skill instructions, tool boundaries, and the complete packaged catalog shared with terminal clients.",
    steps: [
