...
```bash
export IMAGE="cr.eu-north1.nebius.cloud/<registry-id>/models/bionemo-agent:3.2.2"
...
export IMAGE="cr.eu-north1.nebius.cloud/<registry-id>/ba:3.2.2-<digest8>"
...
```bash
npm test
docker build -t bionemo-agent:3.2.2-test .
docker run --rm bionemo-agent:3.2.2-test doctor
```
