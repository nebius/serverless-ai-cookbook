...
```bash
export IMAGE="cr.eu-north1.nebius.cloud/<registry-id>/models/bionemo-agent:3.2.1"
...
export IMAGE="cr.eu-north1.nebius.cloud/<registry-id>/ba:3.2.1-<digest8>"
...
```bash
npm test
docker build -t bionemo-agent:3.2.1-test .
docker run --rm bionemo-agent:3.2.1-test doctor
```
