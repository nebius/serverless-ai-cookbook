/* Create or update one local-login user when deployment credentials are set. */
...
(async () => {
  ...
  try {
    ...
    console.log(`[seed-user] ${result.upsertedId ? 'created' : 'updated'} configured user ${email}`);
  } finally {
    await client.close();
  }
})().catch((error) => {
