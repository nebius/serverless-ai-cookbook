...
Storage and context: `/workspace` is an Object Storage mount, not a full POSIX disk. Use byte copies (`shutil.copyfile` or read_bytes/write_bytes), not copy2/copystat/chmod or rename-based receipt publication. The packaged native, upload and batch clients maintain verified resumable receipts. Read their --help for arguments, not entire implementation sources. Save full inputs, outputs and logs in files; print only the fields needed for the current decision. Read an App schema once per unchanged contract. Preserve long-running operation IDs in Runs rather than spending a turn on repeated polls.

An observation deadline is never a completed stage. Do not chain batch commands after a 30-second wait and claim they all launched. For a sequential multi-App batch study use `/opt/scientific-client/bin/python /opt/bionemo/scientific-workflow.py --help`: its immutable JSON plan contains `schema: scientific-workflow/v1` and `steps`, each with unique `id` and the existing batch client's required named arguments using underscores (model, tool, operation, source, media_type, entry_name, semantic_type, parameters, output, idempotency_key, display_name; optional compression and service_class). Paths are absolute. The workflow resumes each exact receipt, waits until verified before the next step, and visibly waits on an explicit concurrency rejection. For existing partial studies reuse their original output directories and idempotency keys. Run a long-lived workflow with `--wait-seconds 0` in one execution job with timeout 0 and poll that saved execution job; do not launch a second copy. Incomplete bounded observations exit 75, not success. Failed or ambiguous admissions need inspection, never new keys or silent retries. Optional `/v1/me` describes policy, not reserved availability. A dependent scientific analysis still requires checking actual previous output and scientific validity before creating a later plan.
