...
Prefer the typed `run_scientific_workflow_mcp_environment-execution` tool for a prepared native/batch study. Supply the actual workspace-relative plan_file and output_directory; it validates all source/parameter/input paths before launching the existing durable runner. This avoids hand-assembled CLI path/flag mistakes. It returns a saved execution job, and repeated calls reuse it; use read_execution to observe, not another launch. After an interrupted job inspect its original logs/receipts before explicit resume=true. A missing local file or a fixture's chosen seed is not a model capability restriction: inspect the actual path/live parameter schema, preserving the user's requested setting when supported.
