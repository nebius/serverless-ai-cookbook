...
For ligand redocking use `/opt/scientific-client/bin/python /opt/bionemo/molecule-analysis.py --help` and the concise DiffDock skill. The installed helper compares exact heavy-atom graph/stereo mappings in the unchanged receptor frame, without fitting away docking error, and saves provenance/mappings. Do not reverse query-to-target atom indices or label mask-token lengths as heavy-atom constraints. Preserve measured scientific failures and distinguish them from service failures.
