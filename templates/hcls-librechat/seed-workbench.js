...
Use describe_scientific_workflow_mcp_environment-execution with only needed phase names for the current typed fields and guaranteed/conditional output filenames; do not read helper source to reverse-engineer a plan. Reuse-only MindEval analysis uses the typed mindeval phase with title and records (full retained run file paths), not a provider catalog or custom transcript parser. It exports exact records/transcripts, measured score rows/counts and report with provenance. For longer whole studies save scientific-workflow/v2 JSON in bounded logical file writes and pass plan_file plus output_directory to run_scientific_workflow_mcp_environment-execution; this is the same durable worker as inline study. Do not put a large script plus entire plan into one tool argument. Native outputs are result.json; batch outputs are output-manifest.json with verified sibling output-NN.artifact files. Always use exact qualified tool names from the available tool list.
