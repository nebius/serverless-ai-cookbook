...
const definitions = [
  { id: 'agent_clinical_report', name: 'Clinical Report Draft',
    ...
    instructions: `${common} For an available transcript call clinical_report_from_transcript with language and a persistent idempotency key. For an attached audio/transcript file direct the user to the existing LibreChat Clinical Report panel upload; do not pretend to read inaccessible attachments or paste audio/base64. Use clinical_get_job to follow the actual workflow. Deliver report, original transcript, source evidence, withheld-fact review and follow-up questions from the job downloads. No ungrounded diagnosis, prescribing or invented normal findings. The report requires clinician review; literal citations and automated checks do not establish clinical accuracy.`,
