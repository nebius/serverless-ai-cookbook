...
const definitions = [
  ...
  ['workbench_compare_docking', 'Analyze saved docking poses with the installed tested deterministic helper: exact heavy-atom graph/stereochemistry mapping, symmetry-aware RMSD in the unchanged receptor frame, NO ligand fitting. Supply one reference SDF and either saved DiffDock result JSON or predicted SDF; paths are workspace-relative. Explicitly confirm same_coordinate_frame=true. No inference. Returns actual ranked metrics, source hashes, method identity and retained full metric/mapping files. Use these values in the final report rather than writing a new atom-mapping calculator; non-comparable chemistry stays visible.', schema({ reference_file: string, result_file: string, prediction_file: string, same_coordinate_frame: { type: 'boolean', const: true } }, ['reference_file', 'same_coordinate_frame'])],
  ['workbench_compare_structures', 'Analyze saved protein structures with the installed tested deterministic helper: explicit reference:prediction chain mapping, sequence-matched C-alpha fitted RMSD/coverage and mapped-residue contacts, NOT DockQ/CAPRI. Supply workspace-relative reference PDB/CIF and either prediction PDB/CIF or saved result JSON. For redesigned sequences use a scientifically justified hash-bound scientific-residue-correspondence/v1 file instead of pretending sequences match. Returns actual metric and residue-map files with input/output hashes. No inference. Quote saved metrics, keeping per-chain and global fits distinct.', schema({ reference_file: string, result_file: string, prediction_file: string, chain_map: array, residue_mapping_file: string, structure_index: { type: 'integer', minimum: 0, default: 0 } }, ['reference_file', 'chain_map'])],
