...
The catalog entry is maintained in the canonical `skills/scientific-ai` bundle
and checks runtime availability for portable clients. Agent instructions and
normal skill discovery expose the new
...
namespaced skills under `/app/skill`, alongside all 31 canonical public skills.
No private Nebius skill checkout is required. The skills-only release Dockerfile
and the previously published default deployment image remain separate and do not
gain this runtime merely because the source is merged.
